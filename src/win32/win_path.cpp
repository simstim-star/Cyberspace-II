...
std::wstring
Win32FullPath(std::wstring_view subPath)
{
...
std::wstring
Win32GetFileNameOnly(std::wstring_view fullPath)
{
	fs::path p(fullPath);
...
std::string
Win32AppendFileNameToPath(std::wstring_view basePath, std::string_view fileName)
{
	fs::path p(basePath);
	p /= fileName;
...
void
Win32RemoveAllAfterLastSlash(std::wstring &pathBuffer)
{
